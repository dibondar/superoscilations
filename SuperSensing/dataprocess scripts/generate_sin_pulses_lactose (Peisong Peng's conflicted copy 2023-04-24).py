...
path = '/Users/ppsap/Dropbox/SO project/artificial pulses for low resonance'
...
Et.append(np.cos(2*pi*0.5*time)*np.exp(-(time+0)**2/sigma**2)*1)
Et.append(np.cos(2*pi*0.6*time)*np.exp(-(time+0)**2/sigma**2)*1)
Et.append(np.cos(2*pi*0.7*time)*np.exp(-(time+0)**2/sigma**2)*1)
Et.append(np.cos(2*pi*0.8*time)*np.exp(-(time+0)**2/sigma**2)*1)
# Et.append(np.sin(2*pi*0.9*time)*np.exp(-(time+10)**2/sigma**2)*1)
# Et.append(np.sin(2*pi*1.0*time)*np.exp(-(time+10)**2/sigma**2)*1)
...
for i in range(0,4):
    ...
    Et2_ty.append(np.real(bf.array_ifftx(time, lactose_trans(freq, Ef, omega0=1.43))))
    Et2_tx.append(np.zeros(len(Et2_ty[i])))
    Et2s_ty.append(np.real(bf.array_ifftx(time, lga_trans(freq, Ef, omega0=1.43)))[idx1:idx2])
...
for i in range(0,4):
    ...
    for j in range(0,3):
        filename0 = path+'/input_field_F0'+str(i+5)+'_'+str(j+1)+'.dat'
        filename = path+'/P_N0_F0'+str(i+5)+'_'+str(j+1)+'.dat'
        filename2 = path+'/P_N2_F0'+str(i+5)+'_'+str(j+1)+'.dat'
        filename3 = path+'/P_N0S_F0'+str(i+5)+'_'+str(j+1)+'.dat'
        filename4 = path+'/P_N2S_F0'+str(i+5)+'_'+str(j+1)+'.dat'
