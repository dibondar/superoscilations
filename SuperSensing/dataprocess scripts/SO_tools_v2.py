...
class SO_init:
    ...
    def __init__(self, root_window, datapath, x_avg, t_avg):
        ...
        button_optimal = ttk.Button(self.window_so_calculate, text = 'calculate max ND', command = self.calculate_optimal_phase)
        ...
        button_J_varwindow = ttk.Button(self.window_so_discriminate, text = 'cal J varwindow', command = self.max_J_var_window)
        ...
        self.progressbar = ttk.Progressbar(self.window_so_status, orient='horizontal', length=50, mode='determinate')
    ...
    def shift_wfs(self):
        ...
        if SO_switch == 0 and OP_switch == 0:
    ...
    def simulate_so(self):
        ...
        # if 'SO_shifted_' in self.bf.find_str_common(self.filenames_wf_0):
        #     newkey_so0 = 'SO_simulated_' + self.bf.find_str_common(self.filenames_wf_0)
        #     self.x[newkey_so0] = sum(self.x[key] for key in self.filenames_wf_0)
        #     self.t[newkey_so0] = self.t[self.filenames_wf_0[0]]
        #     zero_filler = np.zeros_like(self.x[newkey_so0])
        #     spec = np.stack((self.t[newkey_so0],self.x[newkey_so0],zero_filler), axis = 1)
        #     np.savetxt(self.path + newkey_so0 + '_1.dat', spec)
        # if 'OP_shifted_' in self.bf.find_str_common(self.filenames_wf_0):
        #     newkey_so0 = 'OP_simulated_' + self.bf.find_str_common(self.filenames_wf_0)
        #     self.x[newkey_so0] = sum(self.x[key] for key in self.filenames_wf_0)   
        #     self.t[newkey_so0] = self.t[self.filenames_wf_0[0]]
        #     zero_filler = np.zeros_like(self.x[newkey_so0])
        #     spec = np.stack((self.t[newkey_so0],self.x[newkey_so0],zero_filler), axis = 1)
        #     np.savetxt(self.path + newkey_so0 + '_1.dat', spec)
        # if 'SO_shifted_' in self.bf.find_str_common(self.filenames_wf_1):
        #     newkey_so1 = 'SO_simulated_' + self.bf.find_str_common(self.filenames_wf_1)
        #     self.x[newkey_so1] = sum(self.x[key] for key in self.filenames_wf_1)
        #     self.t[newkey_so1] = self.t[self.filenames_wf_1[0]]
        #     zero_filler = np.zeros_like(self.x[newkey_so1])
        #     spec = np.stack((self.t[newkey_so1],self.x[newkey_so1],zero_filler), axis = 1)
        #     np.savetxt(self.path + newkey_so1 + '_1.dat', spec)
        # if 'OP_shifted_' in self.bf.find_str_common(self.filenames_wf_1):
        #     newkey_so1 = 'OP_simulated_' + self.bf.find_str_common(self.filenames_wf_1)
        #     self.x[newkey_so1] = sum(self.x[key] for key in self.filenames_wf_1)   
        #     self.t[newkey_so1] = self.t[self.filenames_wf_1[0]] 
        #     zero_filler = np.zeros_like(self.x[newkey_so1])
        #     spec = np.stack((self.t[newkey_so1],self.x[newkey_so1],zero_filler), axis = 1)
    ...
    #     for key in self.filenames_wf_0:
    #         if 'OP_shifted_' in key:
    #             newkey_op0 = 'OP_simulated_0' 
    #             op_sum0 = op_sum0 + self.shift_wf0[key]
    #         if 'SO_shifted_' in key:
    #             newkey_so0 = 'SO_simulated_0'
    #             so_sum0 = so_sum0 + self.shift_wf0[key]
    #     if type(op_sum0) != int:
    #         self.x[newkey_op0] = op_sum0
    #         self.t[newkey_op0] = self.t[self.filenames_wf_0[0]]
    #         zero_filler = np.zeros_like(op_sum0)
    #         spec = np.stack((self.t[newkey_op0], op_sum0, zero_filler))
    #         np.savetxt(self.path + newkey_op0 + '_1.dat', spec)
    #     if type(so_sum0) != int:
    #         self.x[newkey_so0] = so_sum0
    #         self.t[newkey_so0] = self.t[self.filenames_wf_0[0]]
    #         zero_filler = np.zeros_like(so_sum0)
    #         spec = np.stack((self.t[newkey_so0], so_sum0, zero_filler), axis = 1)
    #         np.savetxt(self.path + newkey_so0 + '_1.dat', spec)
        
    #     for key in self.filenames_wf_1:
    #         if 'OP_shifted_' in key:
    #             newkey_op1 = 'OP_simulated_1' 
    #             op_sum1 = op_sum1 + self.shift_wf1[key]
    #         if 'SO_shifted_' in key:
    #             newkey_so1 = 'SO_simulated_1'
    #             so_sum1 = so_sum1 + self.shift_wf1[key]
                
    #     if type(op_sum1) != int:
    #         self.x[newkey_op1] = op_sum1
    #         self.t[newkey_op1] = self.t[self.filenames_wf_1[0]]
    #         zero_filler = np.zeros_like(op_sum0)
    #         spec = np.stack((self.t[newkey_op1], op_sum1, zero_filler))
    #         np.savetxt(self.path + newkey_op1 + '_1.dat', spec)
    #     if type(so_sum1) != int:
    #         self.x[newkey_so1] = so_sum1
    #         self.t[newkey_so1] = self.t[self.filenames_wf_1[0]]
    #         zero_filler = np.zeros_like(op_sum0)
    #         spec = np.stack((self.t[newkey_so1], so_sum1, zero_filler), axis = 1)
    #         np.savetxt(self.path + newkey_so1 + '_1.dat', spec)
    ...
    def calculate_optimal_phase(self):
        self.status.set('calculating time delays, please wait...')
        self.window_so_status.update()
        self.so_optimal_go = Calculate_optimal_phase(self.path, self.filenames_wf_0, self.filenames_wf_1)
        self.t_left = self.t[self.filenames_wf_0[0]][0]
        self.t_right = self.t[self.filenames_wf_0[0]][-1]
        self.status.set('SO phases calculated')
    ...
    def max_J_var_window(self):
        ...
        self.window_so_status.update()
        half_window_len = np.arange(0.2, 6, step = 0.1)
        
        for idx, h_win in enumerate(half_window_len):
            self.so_optJ_go = Calculate_optimal_phase_varwindow(self.path, self.filenames_wf_0, self.filenames_wf_1, 
                                                            window = (-h_win, h_win))
            self.progressbar['value'] = float(idx/(len(half_window_len)-1)*100)
            self.J_max.append(self.so_optJ_go.intensity_without_ampl_modulation[0])
            self.window_so_status.update()
...
class Calculate_SO_phase:
    def __init__(self, path, filenames_wf_base, filetype = '.dat'):
        self.pulses = {}
        self.max_ampl = []
...
class Calculate_optimal_phase:
    def __init__(self, path, fname0, fname1, filetype = '.dat'):
        ...
        # Pool()
        pool = Pool(processes = int(cpu_count()/2))
        # self.rand_initial_guess_all = set(zip(self.rand_initial_guess_0, self.rand_initial_guess_1))
...
class Calculate_optimal_phase_varwindow:
    def __init__(self, path, fname0, fname1, window, filetype = '.dat'):
        ...
        #print(bounds_mid)
        self.rand_initial_guess_0 = np.array([
            np.random.uniform(_[0], _[1], 20 * cpu_count()) for _ in self.bounds
        ])
        self.rand_initial_guess_0 = set(tuple(_) for _ in self.rand_initial_guess_0.T)
